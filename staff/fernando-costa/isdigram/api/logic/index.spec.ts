        ...
        it('fails orphan post', done => {
            db.users.deleteAll(error => {
                ...
                db.posts.deleteAll(error => {
                    ...
                    db.users.insertOne({ name: 'Pepe Roni', birthdate: '2000-01-01', email: 'pepe@roni.com', password: '123qwe123' }, (error, insertedUserId) => {
                        if (error) {
                            done(error)
                        ...
                        const insertedPost1 = { author: insertedUserId, image: `http://images.con/${count}`, text: `hello post ${count}`, date: new Date().toLocaleDateString('en-CA') }
                        ...
                        db.posts.insertOne(insertedPost1, (error, insertedPostId1) => {
                            ...
                            db.posts.insertOne(insertedPost2, (error, insertedPost2) => {
                                if (error) {
                                    done(error)
                                ...
                                const insertedPost3 = { author: 'unkonwn-user-id', image: `http://images.com/${count}`, text: `hello post`, date: new Date().toLocaleDateString('en-CA') }

                                db.posts.insertOne(insertedPost3, (error, insertedPost3) => {
                                    if(error) {
                                        done(error)
